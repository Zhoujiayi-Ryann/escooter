...
open class GenPagesOrderDetailsOrderDetailsHome : BasePage {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesOrderDetailsOrderDetailsHome) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!;
            val _ctx = __ins.proxy as GenPagesOrderDetailsOrderDetailsHome;
            ...
            fun genFormatDurationFn(startStr, endStr): String {
            ...
            onLoad1(fun(options){
                ...
                if (!orderId || isNaN(orderId)) {
                    uni_showToast(ShowToastOptions(title = "Order ID is invalid", icon = "none"));
                    return;
                }
                ...
                return wrapUTSPromise(suspend w@{
                        try {
                            ...
                            if (res.code === 1 && res.data) {
                                ...
                                if (!lat || !lng) {
                                    uni_showToast(ShowToastOptions(title = "Invalid vehicle location information", icon = "none"));
                                    return@w;
                                }
                                val totalCost = (data.cost ?: 0) + (data.extended_cost ?: 0);
                                val totalDuration = (data.duration ?: 0) + (data.extended_duration ?: 0);
                                val finalEndTime = data.new_end_time || data.end_time;
                                order.value = object : UTSJSONObject() {
                                    var orderId = data.order_id
                                    var scooterId = data.scooter_id
                                    var startTime = formatToCST(data.start_time)
                                    var endTime = formatToCST(finalEndTime)
                                    ...
                                    var cost = totalCost
                                    var address = data.address || "Unknown"
                                ...
                                uni_showToast(ShowToastOptions(title = res.msg || "Order retrieval failed", icon = "none"));
                            }
                        }
                         catch (err: Throwable) {
                            console.error("🚨 Network error:", err);
                            uni_showToast(ShowToastOptions(title = "Network malfunction", icon = "none"));
                        }
                });
            }
            ;
                ...
                markers.value = utsArrayOf(
                    object : UTSJSONObject() {
                        ...
                        var callout = object : UTSJSONObject() {
                            var content = order.value.scooterLocation
                            var display = "ALWAYS"
            ...
            ;
            val goToSettings = fun(){
                uni_navigateTo(NavigateToOptions(url = "/pages/settings/my_settings/my_settings"));
            }
            ;
            val goToFeedback = fun(){
            ...
            return fun(): Any? {
                ...
                return createElementVNode("view", utsMapOf("class" to "container"), utsArrayOf(
                    ...
                    createElementVNode("view", utsMapOf("class" to "person-btn", "onClick" to goToSettings), utsArrayOf(
                        createVNode(_component_van_icon, utsMapOf("name" to "manager", "class" to "person-icon"))
                    )),
                        ...
                        return utsArrayOf(
                            createElementVNode("view", utsMapOf("class" to "order-details"), utsArrayOf(
                                createElementVNode("view", utsMapOf("class" to "order-info"), utsArrayOf(
                                    createElementVNode("text", utsMapOf("class" to "order-price"), "Completed"),
                                    createElementVNode("view", utsMapOf("class" to "order-tags"), utsArrayOf(
                                        ...
                                        createElementVNode("view", utsMapOf("class" to "status-tag"), "Completed")
                                    ))
                                )),
                                createElementVNode("view", utsMapOf("class" to "info-box"), utsArrayOf(
                                    ...
                                    createElementVNode("view", utsMapOf("class" to "info-item"), utsArrayOf(
                                        ...
                                        createElementVNode("text", utsMapOf("class" to "value"), toDisplayString(order.value.startTime), 1)
                                    )),
                                    createElementVNode("view", utsMapOf("class" to "info-item"), utsArrayOf(
                                        createElementVNode("text", utsMapOf("class" to "label"), "End Time:"),
                                        createElementVNode("text", utsMapOf("class" to "value"), toDisplayString(order.value.endTime), 1)
                                    )),
                                    createElementVNode("view", utsMapOf("class" to "info-item"), utsArrayOf(
                                    ...
                                    createElementVNode("view", utsMapOf("class" to "info-item"), utsArrayOf(
                                        createElementVNode("text", utsMapOf("class" to "label"), "Cost:"),
                                ...
                                createElementVNode("view", utsMapOf("class" to "button-group"), utsArrayOf(
                                    ...
                                    createVNode(_component_van_button, utsMapOf("class" to "half-button feedback-btn", "type" to "primary", "onClick" to goToFeedback), utsMapOf("default" to withSlotCtx(fun(): UTSArray<Any> {
                                        return utsArrayOf(
                                            " Give Feedback "
                                        );
                                    }
