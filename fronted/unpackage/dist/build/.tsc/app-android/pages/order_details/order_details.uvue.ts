import { ref } from "vue";
import { onLoad } from "@dcloudio/uni-app"; // ✅ 使用 UniApp 路由生命周期
import { orderApi } from "@/utils/api/order";

// 🚲 订单信息

const __sfc__ = defineComponent({
  __name: 'order_details',
...
// 弹出面板状态（用于替代 van-floating-panel）
const showPanel = ref(true); // 可扩展用来控制显示
const anchors = [120, Math.round(0.4 * window.innerHeight), Math.round(0.55 * window.innerHeight)];
...
const fetchOrderDetail = async (orderId) => {
  try {
    ...
    uni.showToast({ title: 'Network malfunction, please try again later', icon: 'none' });
  }
};
